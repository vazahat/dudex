...
class BASE_CMP_CommentsForm extends OW_Component
{
    ...
    public function __construct( $entityType, $entityId, $displayType, $pluginKey, $ownerId, $commentCountOnPage, $id, $cmpContextId, $formName )
    {
        ...
        if ( BOL_TextFormatService::getInstance()->isCommentsRichMediaAllowed() )
        {
            ...
            $js = UTIL_JsGenerator::newInstance()->newObject(
                array('ATTP.CORE.ObjectRegistry', $attControlUniq),
                'ATTP.AttachmentsControl',
                array(
                    $cmpContextId,
                    array(
                        'attachmentId' => $attachmentsId,
                        'attachmentInputId' => $attch->getId(),
                        'inputId' => $textArea->getId(),
    ...
    public function onBeforeRender()
    {
        ...
        $this->setTemplate(OW::getPluginManager()->getPlugin('attachments')->getCmpViewDir() . 'comments_form.html');
    }
}