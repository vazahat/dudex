 ...
 *
 */ 
// if ( !class_exists('phpFlickrYNSSApiException') ) {
	// class phpFlickrYNSSApiException extends Exception {
	// }
// }

if ( !class_exists('phpFlickrYNSS') ) {
	if (session_id() == "") {
		@session_start();
	}
	...
	class phpFlickrYNSS {
		...
		function phpFlickrYNSS ($api_key, $secret = NULL, $die_on_error = false) {
		...
		function setProxy ($server, $port) {
			// Sets the proxy for all phpFlickrYNSS calls.
		...
		function getUrl($perms = "read", $remember_uri = true) {
		...
		To use the phpFlickrYNSS::call method, pass a string containing the API method you want
		...
		implemented in phpFlickrYNSS yet.
		...
		*/
		...
		function photosets_getList ($user_id = NULL, $page = NULL, $limit= NULL) {
			/* http://www.flickr.com/services/api/flickr.photosets.getList.html */
			$this->request("flickr.photosets.getList", array("user_id" => $user_id,'page'=>$page,'per_page'=>$limit));
...
if ( !class_exists('phpFlickr_pager',false) ) {
	class phpFlickr_pager {
		var $phpFlickrYNSS, $per_page, $method, $args, $results, $global_phpFlickr;
		var $total = null, $page = 0, $pages = null, $photos, $_extra = null;
		
		
		function phpFlickr_pager($phpFlickrYNSS, $method = null, $args = null, $per_page = 30) {
			$this->per_page = $per_page;
			$this->method = $method;
			$this->args = $args;
			$this->set_phpFlickr($phpFlickrYNSS);
		}
		
		function set_phpFlickr($phpFlickrYNSS) {
			if ( is_a($phpFlickrYNSS, 'phpFlickrYNSS') ) {
				$this->phpFlickrYNSS = $phpFlickrYNSS;
				if ( $this->phpFlickrYNSS->cache ) {
		...
		function load($page) {
			...
			if ( $this->phpFlickrYNSS->cache ) {
				...
				if ( floor($min/500) == floor($max/500) ) {
					...
					$this->results = $this->phpFlickrYNSS->call($this->method, $this->args);
					if ( $this->results ) {
