 ...
 */
class EQUESTIONS_CMP_FriendsFeed extends EQUESTIONS_CMP_Feed
{
    public function findFeed( $startStamp, $count, $questionIds, $order )
    {
        if ( $order == EQUESTIONS_CMP_Feed::ORDER_LATEST )
        {
            return $this->service->findFriendsFeed($startStamp, $this->userId, $count, $questionIds);
        }

        return $this->service->findOrderedFriendsFeed($startStamp, $this->userId, $count, $questionIds, array(
    ...
    {
        return $this->service->findFriendsActivity($startStamp, $this->userId, $questionIds);
    }

    public function findFeedCount( $startStamp )
    {
        return $this->service->findFriendsFeedCount($startStamp, $this->userId);
    }
