...
function vcard_to_string($ba,$encoding,$charset) {
    if ($encoding!=null) {
...
function vcard_to_binary ($ba, $encoding) {
    if ($encoding!=null) {
        $encoding = strtoupper($encoding);
...
class VCardField {
    ...
    function VCardField ($rawName, $rawValue) {
    ...
    function getBinaryValue () {
        $encoding = $this->getFirstParamValue('ENCODING');
         return vcard_to_binary($this->rawValue,$encoding);
    }
    
    function getStringValue () {
        $encoding = $this->getFirstParamValue('ENCODING');
        $charset = $this->getFirstParamValue('CHARSET');
         return vcard_to_string($this->rawValue,$encoding,$charset);
    }
    ...
    function getStringValues ($delim) {
        ...
        $charset = $this->getFirstParamValue('CHARSET');
        $al = array();
        $a = vcard_split($this->rawValue,$delim);
        foreach ($a as $v)
             $al[] = vcard_to_string($v,$encoding,$charset);
        return $al;
    }
}
...
//Tokenizer to tokenize vcard like formats, returning series of VCardField tokens
class VCardTokenizer {
                ...
                if ($hitColon) {
                    ...
                    if (($c==' ' || $c=='\t') && $this->previousKey!=null) {
                        ...
                        if ($this->previousKey!=null) 
                            $field = new VCardField($this->previousKey, $this->previousValue);
                        $this->previousKey = $ba;
                        $this->previousValue = '';
                        $this->readUntilCRLF($this->previousValue);
                        ...
                        // again..
                        $tmpf = new VCardField($this->previousKey, null);
