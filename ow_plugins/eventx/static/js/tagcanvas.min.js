 ...
 */
(function(I) {
    var R, Q, G = Math.abs, r = Math.sin, h = Math.cos, z = Math.max, V = Math.min, B = {}, C = {}, D = {
        0: "0,",
        1: "17,",
        2: "34,",
        3: "51,",
    ...
    function n(Y, aa, af, ac) {
        var ab, ae, j, ad, ag = [], Z = Math.PI * (3 - Math.sqrt(5)), X = 2 / Y;
        for (ab = 0; ab < Y; ++ab) {
            ae = ab * X - 1 + (X / 2);
            j = Math.sqrt(1 - ae * ae);
            ad = ab * Z;
    ...
    function U(Z, X, ac, ai, ah, af, ae, ad, ab) {
        var ag, aj = [], aa = Math.PI * (3 - Math.sqrt(5)), Y = 2 / Z;
        for (af = 0; af < Z; ++af) {
            ae = af * Y - 1 + (Y / 2);
            ag = af * aa;
            ad = h(ag);
    ...
    function p(aa, i) {
        var Z = aa, Y, X, j = (i * 1).toPrecision(3) + ")";
        if (aa[0] === "#") {
            if (!B[aa]) {
                if (aa.length === 4) {
                    B[aa] = "rgba(" + D[aa[1]] + D[aa[2]] + D[aa[3]]
                } else {
                    B[aa] = "rgba(" + C[aa.substr(1, 2)] + C[aa.substr(3, 2)] + C[aa.substr(5, 2)]
                }
            }
            Z = B[aa] + j
        } else {
    ...
    function W(ae, j) {
        var X = 1024, aa = ae.weightGradient, Z, ac, Y, ad, ab;
        if (ae.gCanvas) {
    ...
    function u(aa, Z, X, ad, ab, ac, j) {
        var Y = (ac || 0) + (j && j[0] < 0 ? G(j[0]) : 0), i = (ac || 0) + (j && j[1] < 0 ? G(j[1]) : 0);
        aa.font = Z;
        aa.textBaseline = "top";
        aa.fillStyle = X;
    ...
    function m(aj, ab, af, ah, aa, X, ad, ae, j, ai, ag) {
        var Y = ah + G(j[0]) + ae + ae, i = aa + G(j[1]) + ae + ae, Z, ac;
        Z = g(Y + ai, i + ag);
        if (!Z) {
            return null
        }
    ...
    function P(ac, af, ag, Y) {
        var ah = G(Y[0]), ad = G(Y[1]), Z = ac.width + (ah > ag ? ah + ag : ag * 2), j = ac.height + (ad > ag ? ad + ag : ag * 2), ab = (ag || 0) + (Y[0] < 0 ? ah : 0), X = (ag || 0) + (Y[1] < 0 ? ad : 0), aa, ae;
        aa = g(Z, j);
        if (!aa) {
            return null
        }
    ...
    function K(aj, ab, ah) {
        var ai = parseInt(aj.length * ah), aa = parseInt(ah * 2), Y = g(ai, aa), ae, j, Z, ad, ag, af, X, ac;
        if (!Y) {
            return null
        }
        ae = Y.getContext("2d");
    ...
    function O(aa, ac, Z, X) {
        var Y = X.taglist, ab = X.imageScale, j;
    ...
    function M(Y, X) {
        var j = l.defaultView, i = X.replace(/\-([a-z])/g, function(Z) {
    ...
    function k(X) {
        L(X);
        var j = X.target || X.fromElement.parentNode, i = s.tc[j.id];
        i && (i.mx = i.my = -1)
    }
    function L(Z) {
        var X, j, aa, Y = Z.target && N(Z.target.id) ? Z.target.id : Z.srcElement.parentNode.id;
        for (X in s.tc) {
            j = s.tc[X];
            if (j.tttimer) {
        ...
        if (Y && s.tc[Y]) {
            ...
            if (N(Z.offsetX)) {
                ...
                aa = I(j.canvas).offset();
                if (Z.pageX) {
                    j.mx = Z.pageX - aa.left;
                    j.my = Z.pageY - aa.top
                }
            }
            j.drawn = 0
    ...
    function q(Y) {
        var j = s, i = l.addEventListener ? 0 : 1, X = Y.target && N(Y.target.id) ? Y.target.id : Y.srcElement.parentNode.id;
    ...
    function T(X) {
        var i = s, j = X.target && N(X.target.id) ? X.target.id : X.srcElement.parentNode.id;
        if (j && i.tc[j]) {
            X.cancelBubble = true;
            X.returnValue = false;
    ...
    e.Update = function(ad, ac, ae, aa, ab, j, Z, i) {
        var X = this.tc.outlineOffset, Y = 2 * X;
        this.x = ab * ad + Z - X;
        this.y = ab * ac + i - X;
        this.w = ab * ae + Y;
        this.h = ab * aa + Y;
    ...
    e.DrawColourImage = function(ac, af, ad, ag, ab, i, aj, j, ae) {
        var ah = ac.canvas, Z = ~~z(af, 0), Y = ~~z(ad, 0), aa = V(ah.width - Z, ag) + 0.5 | 0, ai = V(ah.height - Y, ab) + 0.5 | 0, X;
        if (y) {
    ...
    e.DrawPulsate = function(aa, i, j, Y) {
        var Z = new Date().valueOf() - this.ts, X = this.tc;
    ...
    J.Measure = function(ab, j) {
        ...
        if (j.txtOpt) {
            var Y = j.txtScale, Z = Y * this.textHeight, aa = Z + "px " + this.textFont, X = [Y * j.shadowOffset[0], Y * j.shadowOffset[1]], i;
            ab.font = aa;
    ...
    J.Weight = function(Y, X) {
        var j = this.weight, i = X.weightMode;
        this.weighted = true;
        if (i == "colour" || i == "both") {
    ...
    J.DrawText = function(X, ab, j) {
        var ac = this.tc, Z = this.x, Y = this.y, aa, i, ad = this.sc;
    ...
    J.DrawImage = function(Z, af, Y) {
        var ac = this.x, aa = this.y, ag = this.sc, j = this.image, ad = this.w, X = this.h, ab = this.alpha, ae = this.shadow;
        Z.globalAlpha = ab;
    ...
    J.DrawImageIE = function(Z, ad, Y) {
        var j = this.image, ae = this.sc, ac = j.width = this.w * ae, X = j.height = this.h * ae, ab = (this.x * ae) + ad - (ac / 2), aa = (this.y * ae) + Y - (X / 2);
        Z.setTransform(1, 0, 0, 1, 0, 0);
    ...
    J.Calc = function(Z, Y) {
        var i = a(this.p3d, Z), j = this.tc, aa = j.minBrightness, X = j.radius;
        this.p3d = b(i, Y);
    ...
    J.CheckActive = function(Y, ac, X) {
        var ad = this.tc, i = this.outline, ab = this.w, j = this.h, aa = this.x - ab / 2, Z = this.y - j / 2;
        i.Update(aa, Z, ab, j, this.sc, this.p3d, ac, X);
        return i.Active(Y, ad.mx, ad.my) ? i : null
    };

    J.Clicked = function(aa) {
        var j = this.a, X = j.target, Y = j.href, i;
        if (X != "" && X != "_self") {
            if (self.frames[X]) {
                self.frames[X] = Y
            } else {
    ...
    d = s.prototype;
    d.Draw = function() {
        var ag = this.canvas, ae = ag.width, X = ag.height, j = 0, ad = this.yaw, Y = this.pitch, Z = ae / 2, aj = X / 2, ah = this.ctxt, ab, ai, af, ac = -1, al = this.taglist, aa = al.length, ak = this.frontSelect;
    ...
    d.TooltipDiv = function(Y, j) {
        var i = this, X = i.ttdiv.style;
        if (Y && j.title) {
            i.ttdiv.innerHTML = j.title;
            if (X.display == "none" && !i.tttimer) {
                i.tttimer = setTimeout(function() {
                    var Z = I(i.canvas).offset();
                    X.display = "block";
                    X.left = Z.left + i.mx + "px";
                    X.top = Z.top + i.my + 24 + "px";
                    i.tttimer = null
                }, i.tooltipDelay)
            }
        } else {
    ...
    d.Animate = function(ab, Y) {
        var j = this, aa = j.mx, Z = j.my, ad, ac, X, i;
        if (aa >= 0 && Z >= 0 && aa < ab && Z < Y) {
            ...
            if (!j.initial) {
                ad = j.minSpeed, ac = G(j.yaw), X = G(j.pitch);
                if (!j.lx && ac > ad) {
    ...
    d.Clicked = function(Y) {
        var X = this.taglist, i = this.active;
        try {
            if (i && X[i.index]) {
                X[i.index].Clicked(Y)
            }
    ...
    jQuery.fn.tagcanvas = function(X, j) {
        ...
        if (!i.length || !this[0].getContext || !this[0].getContext("2d").fillText) {
            return false
        }
        this.each(function() {
            var ab, Z, ad, ag, ah, ac, af, ae = [], aa = {
                sphere: n,
                vcylinder: w,
                hcylinder: F
            };
            ...
            ac.z1 = (19800 / (Math.exp(ac.depth) * (1 - 1 / Math.E))) + 20000 - 19800 / (1 - (1 / Math.E));
            ac.z2 = ac.z1 * (1 / ac.zoom);
            ac.radius = (this.height > this.width ? this.width : this.height) * 0.33 * (ac.z2 + ac.z1) / (ac.z1);
            ...
            ac.lx = (ac.lock + "").indexOf("x") + 1;
            ac.ly = (ac.lock + "").indexOf("y") + 1;
            ...
            for (ab = 0; ab < i.length; ++ab) {
                ...
                if (ad.length) {
                    ...
                    ac.taglist.push(new E(ac, i[ab].innerText || i[ab].textContent, i[ab], Z[ab], 2, ac.textHeight + 2, ac.textColour || M(i[ab], "color"), ac.textFont || t(M(i[ab], "font-family"))))
                }
                if (ac.weight) {
                    af = x(ac, i[ab]);
                    if (af > ac.max_weight) {
            ...
            s.tc[I(this)[0].id] = ac;
            ac.Tooltip = (ac.tooltip == "native" ? ac.TooltipNative : (ac.tooltip ? ac.TooltipDiv : ac.TooltipNone));
            if (ac.tooltip) {
