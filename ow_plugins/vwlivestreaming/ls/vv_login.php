<?php
        ...
        if (isset ($_COOKIE ['sessionNameLv'])) {
          $sessionName = $_COOKIE ['sessionNameLv'].".vwf";
          $sessionsPath = "sessions/";
          if ($handle = opendir($sessionsPath)) {
              while (false !== ($file = readdir($handle))) {
                if (filectime($sessionsPath.$file) < (time () - 160) && (substr($file,0,3) == 'vlv' ) ) 
                {
        ...
        $userLink=urlencode($params2 [44]);
        $userPicture=urlencode($params2 [45]);
